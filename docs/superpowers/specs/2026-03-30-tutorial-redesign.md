...
Ch03-07 使用同一个项目 **DemoAPI**（Ch03 已有的 Express REST API），Ch08-11 各自使用最适合的场景。

### 原因

- Ch03-07 都是个人开发场景，一个 CRUD API 能自然生成所有需要的痛点
- Ch08-11 涉及多代理/自动化/团队场景，单个 API 无法自然支撑（强行贯穿会硬凑）
- 项目命名为 DemoAPI 而非 TaskFlow，因为现有 Ch03 的领域是社交内容（Users/Posts/Comments），不是任务管理

### TaskFlow 演进

| 章节 | TaskFlow 版本 | 发生了什么 |
|------|--------------|-----------|
| Ch03 | v0 | Vibe Coding 搭出 CRUD API（Users/Posts/Comments），4 轮后风格崩溃（已有内容）。注意：现有 API 是社交内容领域（用户/帖子/评论），项目统称为 **DemoAPI** 而非 TaskFlow，避免领域名称与实际内容不匹配 |
...
1. **回顾 Ch03-09 的 7 次 失败→响应 循环**（Ch03 风格退化、Ch04 session 失忆、Ch05 复杂任务失控、Ch06 重复工作流、Ch07 遗忘检查、Ch08 上下文衰减、Ch09 人工瓶颈），总结规律
...
**补"验证"：**
- 质量标准：同一个 prompt 跑 3 次，检查 (a) 响应格式结构是否一致（如都用 `{data, error}`），(b) 文件命名规范是否一致，(c) ID 生成策略是否一致。3 项全部一致 = 通过
...
**补"失败"开场：**
你每次让 Claude 做发布都要写一大段 prompt 描述步骤（跑测试 → 构建 → 推到预发布环境验证 → 正式发布），而且每次细节略有不同，偶尔漏掉"先在预发布环境验证"这一步。上个月就因为漏了这步直接推到了生产。这个场景适用于任何多步骤工作流（部署、数据库迁移、release 流程等），不限于特定技术栈。
...
#### Ch09 Agent SDK + 程序化接入（结构重组，当前 1582 行 → 目标 1600+）

**注意：** Ch09 已有 1582 行，包含决策树、CLI 基础、SDK 基础、session 管理、输出格式等内容。不是从零重写，而是**在现有基础上重组结构、补失败开场和验证收尾、加深实战部分**。此外，Ch10 中已有的 GitHub Actions CI/CD 示例（`claude-review.yml`）应迁移至 Ch09。

**补"失败"开场：**
你每次手动让 Claude 做 code review 要 10 分钟，一天 20 个 PR 就是 3 小时人力。或者：你度假了一周，回来发现 PR 堆积没人 review — 因为整个流程依赖你坐在终端前。

**重组结构：**

| 节 | 现有内容 | 改动 |
|---|---------|------|
| 9.1 四种接入模式 | 已有决策树 ✓ | 保留，补失败开场 |
| 9.2 CLI 模式实战 | 已有 `claude -p` 基础 | 扩展：加一个完整的批量脚本（扫描 TODO 注释生成报告） |
| 9.3 SDK 最小示例 | 已有部分 SDK 代码 | 补一个 Python PR review bot 最小示例（不写完整 SDK 教程） |
| 9.4 CI/CD 集成 | 部分在 Ch10 | 从 Ch10 迁入 `claude-review.yml`，扩展为完整 GitHub Actions workflow（含 secrets、超时、费用限制） |
| 9.5 定时任务 + Channels | 已有 CronCreate 基础 | 补 Channels（Telegram/Discord）简介 |
| 9.6 无人值守安全 | 已有权限/成本 QualityCallout | 系统化为三件事：超时熔断（`--max-turns`）、预算上限、凭证清洗（`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`） |
...
- **长时运行代理**：Anthropic 的 Initializer + Coding Agent 模式（`claude-progress.txt` 增量保存），简要提及 GAN 三代理架构并附链接（不深展，避免scope creep）

**补"验证"：**
- ROI 决策树：任务 < 2min 直接做，> 5min 且可拆 → 子代理
- 质量标准：用一个具体案例展示 ROI 计算（如"RBAC 重构单 session 用 ~45K tokens，3 个子代理用 ~52K tokens(1.15x)，但输出质量更高因为每个子代理有新鲜上下文"），教读者自己算而非给固定阈值
...
- 开头给 API 命名为 DemoAPI + 受众定位段（Agentic Engineering 术语）（**已提前到 Phase 0**）
- 末尾加痛点→Harness 组件映射预告表（**已提前到 Phase 0**）
...
```
Phase 0 — 前置：贯穿项目命名（Ch04-07 依赖此项）
└── Ch03 补 DemoAPI 命名 + 预告表（必须先于 Phase 1）

Phase 1 — P0：多数读者使用的章节补深度
├── Ch04 CLAUDE.md 扩写（失败开场 + DemoAPI 对比 + 验证标准）
├── Ch06 Skills 扩写（失败开场 + 三选一决策树 + 排障）
├── Ch07 Hooks 扩写（Auto Mode + 5 核心事件教透 + 有效性指标）
└── Ch09 Agent SDK 重组（补失败开场 + 实战加深 + 从 Ch10 迁入 CI/CD + 安全）

Phase 2 — P1：弱章节加厚 + 轻量更新合并
├── Ch08 Subagent 加厚（Agent Teams + Worktree + 长时运行 + ROI）
├── Ch10 工作流重写核心（回顾→归纳→诊断框架，CI/CD 示例已迁出到 Ch09）
├── Ch11 治理扩写（落地路线图 + 成本量化 + 事故响应）
├── Ch01 补 Auto Mode + 迭代过程 + 受众定位
├── Ch02 补受众定位段
└── Ch05 补失败回退 + DemoAPI 连接

Phase 3 — P3：横切改进
├── 行业数据注入各章（使用集中式数据组件，各章引用，便于更新）
├── Harness Engineering 线索标注
└── toc.ts + HomePage 更新
```

**注意事项：**
- Phase 0 必须先完成，因为 Phase 1 的 Ch04-07 会引用 DemoAPI 项目名
- 原 P2（Ch01/02/03/05 轻量更新）合并到 Phase 2，减少阶段数
- Ch09 从"重写"改为"重组"，因为已有 1582 行内容
- Ch10 的 CI/CD 示例（`claude-review.yml`）迁移到 Ch09，需在 Phase 1 中协调
- 行业数据使用集中式组件/文件管理，避免各章散落导致更新困难
...
- **不发明大型方法论框架** — 用 Anthropic 原生的 Harness Engineering 迭代过程，诊断框架（DecisionTree）是从实践中归纳的工具而非先验框架
