...
prefer   → "prefer functional style"`}
        ...
        <div className="mt-4 px-4 py-3 rounded-lg text-xs" style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)' }}>
          <span style={{ color: 'var(--color-text-muted)' }}>
            注：以上权重层级为经验规律，非精确测量。具体遵从率可能随模型版本更新而变化。核心原则——强语言 = 强遵从——是稳定的。
          </span>
        </div>
...
# - 注意：token 成本更高，按需使用`}
      ...
      <section>
        ...
        <p className="mb-4" style={{ color: 'var(--color-text-secondary)' }}>
          Claude Code 的推理深度不是固定的——你可以通过 effort 级别控制它投入多少"思考量"。
          这直接影响回答质量和 token 消耗。
        </p>
          ...
          <table className="w-full text-sm" style={{ color: 'var(--color-text-secondary)' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                <th className="text-left py-2 px-3 font-medium" style={{ color: 'var(--color-text-primary)' }}>级别</th>
                <th className="text-left py-2 px-3 font-medium" style={{ color: 'var(--color-text-primary)' }}>适用场景</th>
                <th className="text-left py-2 px-3 font-medium" style={{ color: 'var(--color-text-primary)' }}>Token 消耗</th>
              </tr>
            </thead>
            <tbody>
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                <td className="py-2 px-3 font-mono text-xs" style={{ color: 'var(--color-tier-l1)' }}>low</td>
                <td className="py-2 px-3">简单查询、格式转换、已知模式的代码生成</td>
                <td className="py-2 px-3">最少</td>
              </tr>
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                <td className="py-2 px-3 font-mono text-xs" style={{ color: 'var(--color-tier-l2)' }}>medium</td>
                <td className="py-2 px-3">日常开发任务、bug 修复、中等复杂度的功能实现</td>
                <td className="py-2 px-3">标准</td>
              </tr>
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                <td className="py-2 px-3 font-mono text-xs" style={{ color: 'var(--color-accent)' }}>high</td>
                <td className="py-2 px-3">架构决策、复杂算法、安全审查</td>
                <td className="py-2 px-3">较多</td>
              </tr>
              <tr>
                <td className="py-2 px-3 font-mono text-xs" style={{ color: 'var(--color-tier-l3)' }}>max</td>
                <td className="py-2 px-3">最复杂的系统设计、多维度权衡</td>
                <td className="py-2 px-3">最多</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <p className="mb-4" style={{ color: 'var(--color-text-secondary)' }}>
          在 prompt 中使用 <code className="font-mono text-xs px-1.5 py-0.5 rounded" style={{ background: 'var(--color-bg-tertiary)', color: 'var(--color-accent)' }}>ultrathink</code> 关键词可以触发高 effort 级别的深度推理。
          Opus 4.6 默认使用 medium effort；当你需要更深入的分析时，在 prompt 中加入"ultrathink"或"think harder"即可。
        </p>

        <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
          后续章节中，Skills 的 frontmatter 可以通过 effort 字段为特定任务预设推理深度（见 Ch06），
          Agent SDK 的参数也支持 effort 级别控制（见 Ch09）。
        </p>
      </section>
      ...
      <ReferenceSection version="Claude Code v1.x">
        <div className="text-sm space-y-2" style={{ color: 'var(--color-text-secondary)' }}>
          <p>关键词力度层级参考表（经验规律，截至 vX.X）</p>
          <p>XML 标签参考列表</p>
          <p>effort 级别 token 消耗估算（待补充）</p>
        </div>
      </ReferenceSection>
    </div>
  )
